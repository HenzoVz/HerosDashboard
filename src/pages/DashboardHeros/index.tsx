...
const DashboardHeros: React.FC = () => {
  ...
  const [heroRepositoriesLocal, setHeroRepositoriesLocal] = useState<CharactersProps[]>(() => {
    
    const storageRepositories = localStorage.getItem('@HerosDashboard:repositories');
  ...
  useEffect(() => {
    localStorage.setItem('@HerosDashboard:repositories', JSON.stringify(heroRepositoriesLocal));
  }, [heroRepositoriesLocal])

  async function handleAddHeroRepository(event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault();
    
    searchCharacter(newHeroRepository).get('').then((response) => {
      const character = response.data;

      setHeroRepositoriesLocal([character.data.results[0], ...heroRepositories]);
